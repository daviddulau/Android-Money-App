            ...
            for (int i = 0; i < this.Entries.Count(); i++)
            {
                ...
                if (!string.IsNullOrEmpty(entry.Label))
                {
                    using (var paint = new SKPaint())
                    {
                        ...
                        //calculate height area
                        float areaHeight = this.SplitLines(text, paint, itemSize.Width).Count() * footerHeight;
                        var area = SKRect.Create(point.X - (itemSize.Width / 2), height - (this.Margin + (this.LabelTextSize / 2)), itemSize.Width, areaHeight);

                        // Background
                        paint.Color = Globals.background;
                        canvas.DrawRect(area, paint);
                        paint.Color = entry.TextColor;

                        //draw
                        this.DrawText(canvas, text, area, paint);
                    }
                }
            }
        }
